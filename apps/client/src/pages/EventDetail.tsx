  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#fff' }}>
      ...
      <div className="event-hero" style={{
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '400px', height: '200px', background: 'radial-gradient(circle, rgba(124,58,237,0.2) 0%, transparent 70%)', borderRadius: '50%' }} />
        <div className="animate-fadeInUp" style={{ position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 'bold', marginBottom: '0.5rem' }}>{event.name}</h1>
          <p style={{ color: '#c084fc', fontSize: '0.95rem' }}>📍 {event.location}</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '800px', margin: '2rem auto', padding: '0 1rem' }}>
        <div className="animate-fadeInUp glass" style={{ borderRadius: '20px', overflow: 'hidden', boxShadow: '0 0 60px rgba(124,58,237,0.15)' }}>
          <div className="event-content" style={{ padding: '2rem' }}>
            <p style={{ color: '#9ca3af', lineHeight: 1.7, marginBottom: '2rem', fontSize: '0.95rem' }}>{event.description}</p>
            ...
            <div className="event-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
              {[
                ...
                <div key={i} className={`animate-fadeInUp delay-${(i + 1) * 100}`} style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '12px', padding: '0.875rem' }}>
                  ...
                  <p style={{ fontWeight: 'bold', color: item.color || '#fff', fontSize: '0.95rem' }}>{item.value}</p>
                </div>
              ))}
            </div>
